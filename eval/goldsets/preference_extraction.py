"""EXTRACT step — derive typed preference/fact/constraint candidates from user turns (NO durable
writes). Ported into the eval bundle as a self-contained, transport-agnostic reference extractor
for `key_correctness_eval.py`'s scoring harness.

PURE and transport-agnostic: the LLM is an INJECTED ``chat_fn``, so the SAME extraction logic can
run under any transport (OpenAI-compatible key, a local codex CLI subprocess, or a fake in tests) —
no eval-only code path, so a measured lift is credible, not a benchmark artifact. This module
performs NO durable writes and NO conflict search; it only turns text into typed candidates. The
caller (the eval harness here; a gated production path elsewhere) decides/queues/finalizes.

Design: a ``canonical_key = "{kind}:{domain}"`` identifies a durable SLOT — two candidates sharing a
canonical_key are about the same slot (an UPDATE/supersede pair when the value differs).
``kind in {preference, fact, constraint}``; ``domain`` is a stable snake_case slot name. Key
STABILITY (same slot -> same key across paraphrases) and the ABSENCE of harmful cross-slot
collisions are the properties `key_correctness_eval.py` measures.
"""
...
# A transport-agnostic chat call: (system_prompt, user_prompt) -> raw model text.
...
class PreferenceCandidate:
    ...
    # The coercion keeps a usable canonical_key (safe), but the eval counts a coerced candidate as a
    # SCHEMA failure so a model that ignores the kind contract is not masked.
...
def _extract_json_array(raw: str) -> list | None:
    """Best-effort parse of a JSON array from a model response (tolerates ```json fences / stray
    prose). Returns the array (possibly EMPTY -- a valid "no attributes" reply) when the model
    returned a JSON array; returns ``None`` when the reply was NOT a JSON array at all (non-array
    JSON / unparseable / prose) -- a contract failure the caller counts. Never raises."""
    text = (raw or "").strip()
    if text.startswith("```"):
...
def parse_with_stats(raw: str, source_sid: str) -> tuple[list[PreferenceCandidate], dict[str, int]]:
    ...
    counting only emitted candidates would mask a model that returns mostly-malformed output.
    ``raw_objects`` is the FULL array length (every element), so nothing is invisible to
    schema_validity."""
    ...
    for item in objs:
        if not isinstance(item, dict):
            ...
            continue
        # Coalesce NULL explicitly: item.get("domain", "") returns None when the key is present with
        # a null value (the default applies only to ABSENT keys), and normalize_domain(None) would
        # yield a bogus "none" slot / str(None) a bogus "None" value -- a schema failure masquerading
        # as a valid candidate. Treat null (or missing) required fields as a drop.
...
def extract_preferences(
    ...
    message short-circuits to ``[]`` without an LLM call. If ``stats`` is given it is populated with
    the raw schema-compliance counts (for the eval's schema_validity); other callers omit it."""
