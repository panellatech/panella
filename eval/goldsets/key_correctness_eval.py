...
def score(
    ...
    rep.counts = {
        ...
        "hr_merged_pairs": hr_merged_pairs,
        "high_risk": hr_total,
...
def run_eval(chat_fn: ChatFn, *, goldset_path: Path = DEFAULT_GOLDSET, fixture_path: Path = DEFAULT_FIXTURE) -> dict:
    ...
    result["report"] = {
        ...
        "key_stability_correct": sum(row["merged_pairs"] for row in rep.per_lifecycle),
        "key_stability_total": sum(row["gold_pairs"] for row in rep.per_lifecycle),
        ...
        "hr_merged_pairs_zero": rep.counts["hr_merged_pairs"] == 0,
